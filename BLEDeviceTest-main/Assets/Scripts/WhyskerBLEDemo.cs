...
//            writeMotorPos.interactable = true;
    ...
    {
        bool isXNumeric = int.TryParse(xInput.text, out int x);
        bool isYNumeric = int.TryParse(yInput.text, out int y);
        ...
        Debug.Log("Write X as " + x + " and Y as " + y);
        GetComponent<CubeBLE>().WriteMotor((byte)x, (byte)y);
    }
